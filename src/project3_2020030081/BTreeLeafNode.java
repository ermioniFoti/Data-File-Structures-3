...
class BTreeLeafNode<TKey extends Comparable<TKey>, TValue> extends BTreeNode<TKey> {
   private static int LEAFORDER = 9;
    ...
    public BTreeLeafNode() {
        ...
        this.values = new List[LEAFORDER + 1];
    }
    
    static void setLeafOrder(int order) {
    	LEAFORDER = order - 1;
    }

    @SuppressWarnings("unchecked")
    public List<TValue> getValue(int index) {
        return (List<TValue>) this.values[index];
    }
...
//    }
    ...
    public int search(TKey key) {
        for (int i = 0; i < this.getKeyCount(); ++i) {
             ...
             SearchStats.increaseCompareCount();
             if (cmp == 0) {
                 return i;
             }
             else if (cmp > 0) {
    ...
    @SuppressWarnings("unchecked")
    public void insertKey(TKey key, List<TValue> values) {
        int index = 0;
        while (index < this.keyCount && ((TKey) this.keys[index]).compareTo(key) < 0)
            ++index;
    ...
    private void insertAt(int index, TKey key, TValue value) {
        ...
        if (index < this.keyCount && ((TKey) this.keys[index]).compareTo(key) == 0) {
            // duplicate key (just add another value)
            ...
            (this.values[index] = new ArrayList<TValue>()).add(value);
    ...
    protected BTreeNode<TKey> split() {
        ...
        for (int i = midIndex; i < this.getKeyCount(); ++i) {
            newRNode.keys[i - midIndex] = this.keys[i];
        ...
        newRNode.keyCount = this.keyCount - midIndex;
    ...
    protected void fusionWithSibling(TKey sinkKey, BTreeNode<TKey> rightSibling) {
        ...
        for (int i = 0; i < siblingLeaf.getKeyCount(); ++i) {
            this.keys[j + i] = siblingLeaf.keys[i];
            //this.setKey(j + i, siblingLeaf.getKey(i));
            this.values[j + i] = siblingLeaf.values[i];
